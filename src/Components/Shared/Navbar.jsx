...
const Navbar = () => {
  ...
  return (
    <nav
      className={`sticky top-0 w-full max-w-7xl mx-auto z-50 transition-all duration-300 rounded-2xl ${
        scrolled
      ...
      <div className="container mx-auto px-6">
        <div className="flex justify-between items-center">
              ...
              <div className="flex items-center gap-4 pl-2">
                ...
                <div className="relative group cursor-pointer z-50">
                  ...
                  <div className="absolute right-0 mt-4 w-60 bg-bg-surface/95 backdrop-blur-xl border border-border-base rounded-2xl shadow-xl shadow-black/5 p-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all transform origin-top-right translate-y-2 group-hover:translate-y-0">
                    ...
                    <Link
                      to="/dashboard"
                      className="flex items-center gap-2 px-4 py-3 rounded-xl text-sm font-semibold text-text-sec hover:bg-bg-elevated hover:text-text-main transition-colors"
                    >
          ...
          <div className="md:hidden flex items-center gap-2">
            <button
              onClick={toggleTheme}
              className="p-2 text-text-sec hover:text-brand hover:bg-bg-elevated rounded-xl transition-colors"
            >
              {theme === "dark" ? (
                <Moon className="w-5 h-5" />
              ) : (
          ...
          {!isAuthenticated ? (
            <div className="grid grid-cols-2 gap-3 pt-2">
              <Link
                to="/login"
                onClick={() => setIsMenuOpen(false)}
                className="w-full text-center py-3.5 rounded-xl border border-border-base text-text-main font-bold hover:bg-bg-elevated transition-colors"
              >
                Login
              </Link>
              <Link
                to="/register"
                onClick={() => setIsMenuOpen(false)}
                className="w-full text-center py-3.5 rounded-xl bg-brand text-text-inv font-bold shadow-lg hover:brightness-110 transition-all"
              >
                Register
              </Link>
            </div>
